...
class VideoPlayerProgressBar(ProgressBar):
    video = ObjectProperty(None)
    seek = NumericProperty(None)
    alpha = NumericProperty(1.)

    def __init__(self, **kwargs):
        super(VideoPlayerProgressBar, self).__init__(**kwargs)
        self.bubble = Factory.Bubble(size=(50, 40))
    ...
    def on_video(self, instance, value):
        self.video.bind(position=self._update_bubble, state=sef._showhide_bubble)
...
class VideoPlayerAnnotation(Label):
    ...
    *bgsource: 'filename' - background image used for the background
...
if __name__ == '__main__':
    ...
    player = VideoPlayer(source=sys.argv[0])
    runTouchApp(player)
    if player:
        player.state = 'stop'
